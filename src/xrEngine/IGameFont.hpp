...
public:
    ...
    virtual void Initialize(const char* shader, const char* texture) = 0;
    ...
    virtual float SizeOf_(const char* s) = 0;
    ...
    virtual void MasterOut(bool bCheckDevice, bool bUseCoords, bool bScaleCoords, bool bUseSkip,
        float _x, float _y, float _skip, const char* fmt, va_list p) = 0;
    ...
    virtual void OutI(float _x, float _y, const char* fmt, ...) = 0;
    virtual void Out(float _x, float _y, const char* fmt, ...) = 0;
    virtual void OutNext(const char* fmt, ...) = 0;
