...
namespace
{
  void RunBenchmark(const char *name)
  {
      ...
      for (u32 i = 0; i<benchmarkCount; i++)
      {
          ...
          Core.Params = (char *)xr_realloc((void *)Core.Params, cmdSize);
          xr_strcpy((char *)Core.Params, cmdSize, benchmarkCommand.c_str());
          xr_strlwr((char *)Core.Params);
          InitInput();
          if (i)
              InitEngine();
          Engine.External.Initialize();
