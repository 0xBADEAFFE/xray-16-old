...
bool CUIXmlInit::InitOptionsItem(CUIXml& xml_doc, LPCSTR path, int index, CUIOptionsItem* pWnd){
	...
	if (xml_doc.NavigateToNode(buf,index))
	{
		...
		if(depends)
		{
			...
			if(0==xr_stricmp(depends,"snd"))
				d = CUIOptionsItem::sdSndRestart;
			else
			if(0==xr_stricmp(depends,"restart"))
				d = CUIOptionsItem::sdSystemRestart;
			else
			if(0==xr_stricmp(depends,"runtime"))
						...
						int index, CUIProgressBar* pWnd)
{
	...
	else if ( xr_stricmp( mode_str, "horz" ) == 0 )	{	mode = CUIProgressBar::om_horz;	}
	else if ( xr_stricmp( mode_str, "vert" ) == 0 )	{	mode = CUIProgressBar::om_vert;	}
	else if ( xr_stricmp( mode_str, "back" ) == 0 )	{	mode = CUIProgressBar::om_back;	}
	else if ( xr_stricmp( mode_str, "down" ) == 0 )	{	mode = CUIProgressBar::om_down;	}
...
void CUIXmlInit::InitAutoStaticGroup(CUIXml& xml_doc, LPCSTR path, int index, CUIWindow* pParentWnd)
{
	...
	while(node)
	{
		...
		if(0==xr_stricmp(node_name,"auto_frameline"))
		{
		...
		if(0==xr_stricmp(node_name,"auto_text"))
		{
			++cnt_text;
		}
