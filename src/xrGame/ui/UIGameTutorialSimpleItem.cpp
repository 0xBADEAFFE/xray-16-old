...
void CUISequenceSimpleItem::Start()
{
	...
	if (g_pGameLevel)
	{
		...
		if (     !xr_stricmp( m_pda_section, "pda_tasks"       ) ) {ui_game_sp->GetPdaMenu().SetActiveSubdialog("eptTasks");		bShowPda = true;	}
		else if( !xr_stricmp( m_pda_section, "pda_ranking"     ) ) {ui_game_sp->GetPdaMenu().SetActiveSubdialog("eptRanking");	bShowPda = true;	}
		else if( !xr_stricmp( m_pda_section, "pda_logs"        ) ) {ui_game_sp->GetPdaMenu().SetActiveSubdialog("eptLogs");		bShowPda = true;	}
