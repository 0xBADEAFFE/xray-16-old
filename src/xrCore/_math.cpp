...
XRCORE_API u64 QPC() noexcept
{
    u64 _dest;
#ifdef CONFIG_USE_SDL
    _dest = SDL_GetPerformanceCounter();
#else // Windows
...
void Detect()
{
    ...
    start = GetCLK();
#ifdef CONFIG_USE_SDL
    while (SDL_GetTicks() - dwStart < 1000);
#else
...
#if defined(LINUX) || defined(M_BORLAND)
...
#endif // defined(LINUX) || defined(M_BORLAND)
...
void __cdecl thread_entry(void* _params)
{
    STUBBED("thread_entry(void* _params)");
}

void thread_spawn(thread_t* entry, const char* name, unsigned stack, void* arglist)
{
    STUBBED("hread_spawn(thread_t* entry, const char* name, unsigned stack, void* arglist)");
}
#endif // WINDOWS
