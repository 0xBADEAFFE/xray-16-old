	...
	_DDS_CUBE:
		{
			...
			dwHeight = Texture.extent().y;
			dwDepth = Texture.extent().z;
			fmt = GL.translate(Texture.format(), Texture.swizzles());
			...
			for (size_t face = 0; face < Texture.faces(); face++)
			{
				for (size_t i = 0; i < mip_cnt; i++)
				{
					...
					{
						CHK_GL(glCompressedTexSubImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + face, i, 0, 0, Texture.extent(i).x, Texture.extent(i).y,
							(GLenum)fmt.Internal, Texture.size(i), Texture.data(0, face, i)));
					}
					else {
						CHK_GL(glTexSubImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + face, i, 0, 0, Texture.extent(i).x, Texture.extent(i).y,
	...
	_DDS_3D:
		{
			...
			for (size_t i = 0; i < mip_cnt; i++)
			{
				if (gli::is_compressed(Texture.format()))
				{
					CHK_GL(glCompressedTexSubImage3D(GL_TEXTURE_3D, i, 0, 0, 0, Texture.extent(i).x, Texture.extent(i).y, Texture.extent(i).z,
						(GLenum)fmt.Internal, Texture.size(i), Texture.data(0, 0, i)));
				}
				else {
					CHK_GL(glTexSubImage3D(GL_TEXTURE_3D, i, 0, 0, 0, Texture.extent(i).x, Texture.extent(i).y, Texture.extent(i).z,
	...
	_DDS_2D:
		{
			...
			for (size_t i = 0; i < mip_cnt; i++)
			{
				if (gli::is_compressed(Texture.format()))
				{
					CHK_GL(glCompressedTexSubImage2D(GL_TEXTURE_2D, i, 0, 0, Texture.extent(i).x, Texture.extent(i).y,
						(GLenum)fmt.Internal, Texture.size(i), Texture.data(0, 0, i)));
				}
				else {
					CHK_GL(glTexSubImage2D(GL_TEXTURE_2D, i, 0, 0, Texture.extent(i).x, Texture.extent(i).y,
						(GLenum)fmt.External, (GLenum)fmt.Type, Texture.data(0, 0, i)));
				}
			}
			FS.r_close(S);
